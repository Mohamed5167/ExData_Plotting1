plot4 <- function(data_file) {
    data_electric <- read.table(data_file, header = TRUE, sep = ";",
                                na.strings = "?")
    data_electric$Date <- as.Date(data_electric$Date, "%d/%m/%Y")
    new_data <- data_electric[data_electric$Date == ymd("2007-02-01")
                              | data_electric$Date == ymd("2007-02-02"), ]
    
    df_date_time <- as.POSIXct(paste(new_data$Date, new_data$Time))
    
    
    png(filename = "plot4.png", width = 480, height = 480)
    par(mfcol = c(2,2))
    
    plot(df_date_time, new_data$Global_active_power, type = "l",
         xlab = "", ylab = "Global Active Power")

    plot(df_date_time, new_data$Sub_metering_1, type = "l", xlab = "",
         ylab = "Energy sub metering")
    lines(df_date_time, new_data$Sub_metering_2, col = "red")
    lines(df_date_time, new_data$Sub_metering_3, col = "blue")
    legend("topright", legend = tail(colnames(new_data), n = 3), lty = 1,
           col = c("black", "red", "blue"))

    plot(df_date_time, new_data$Voltage, type = "l",
         xlab = "datetime", ylab = "Voltage")
    
    plot(df_date_time, new_data$Global_reactive_power, type = "l",
         xlab = "datetime", ylab = "Global_reactive_power")
    dev.off()
}